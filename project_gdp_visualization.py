...
def reconcile_countries_by_name_emtpy(plot_countries, gdp_countries):  # 返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    """
    输入参数:
...
def reconcile_countries_by_name(plot_countries, gdp_countries, year):
    """
      输入参数:
      plot_countries: 绘图库国家代码数据，字典格式，其中键为绘图库国家代码，值为对应的具体国名
      gdp_countries:世行各国数据，嵌套字典格式，其中外部字典的键为世行国家代码，值为该国在世行文件中的行数据（字典格式)
...
def render_world_map(full,empty_always,empty_at_this_time, year, map_file):  # 将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    """
    Inputs:
      gdpinfo:gdp信息字典
    ...
    worldmap_chart.title = '%s World GDP'%(year)
    worldmap_chart.add(year,full)
    worldmap_chart.add("missing from world bank",empty_always)
    worldmap_chart.add("no data at this year",empty_at_this_time)
...
def test_render_world_map(year):  # 测试函数
    ...
    x = reconcile_countries_by_name_emtpy(pygal_countries, reader)#无数据的国家
    y = reconcile_countries_by_name(pygal_countries, reader, year)#输入年份没有数据的国家
    z = build_map_dict_by_name(reader, pygal_countries, year)#一直有数据的国家

    render_world_map(z, x, y, year, "isp_gdp_world_name_%s.svg" % (year))
